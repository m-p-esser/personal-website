---
title: Curriculum Vitae
layout: single
permalink: /cv/
author_profile: true
---

## Marc-Philipp Esser
Junior Consultant for Data Science and Master in Market Research

## Currently

I'm currently working as a consultant for Data Science at KI Performance GmbH which is an IT consulting company that offers solutions in a variety of data centric fields. I’m working in the department for Machine Learning and Artifical Intelligence. I would like to point out that the views presented on this blog are my own and in no way affiliated with my employer.

## Employment

`10/2019 -` 
__KI performance GmbH (Cologne)__ Junior Consultant (Machine Learning, Artifical Intelligence, Robot Process Automation)

`2019/06 - 2019/09` 
__KI performance GmbH (Cologne)__ Master student 

`2018/07 - 2019/05`
__REWE Deutscher Supermrkt AG & Co. KGaA (Cologne)__ Working student Data Analytics / Data Science

`2016/10 - 2018/06`
__Various Market Research institutes (Cologne)__ Working student in Quantitative / Qualitative Market Research

## Education

`2018-2020`
__Technische Hochschule Köln__ Market Research (Master of Science). My thesis was titled _Predicting the popularity of instagram posts with neural networks based on example of the brand Mercedes Benz_ 

`2017-2017`
__Hochschule Wismar__ Business Administration (Bachelor of Arts) with focus on Marketing / Consulting

## Technical skills

* Python
* SQL (Basics)
* Microsoft PowerBI
* UiPath (RPA)
* Git (Basics)
* Azure

## Areas of expertise

* Statistics
* Machine learning
* Data visualisation
* Data analysis

## Projects

### Github

My [personal github account](https://github.com/m-p-esser) hosts my hobby development projects as well as listing contributions to open source tools. Still work in progress, projects will follow soon.

### Blog

I write a data science blog at [the-data-explorer.herokuapp.com](https://the-data-explorer.herokuapp.com/blog/) where I apply Python to open datasets and write-up the results.

